...
const Filter = ({ setFilterVisible }) => {
  ...
  return (
    <div className="absolute right-0 top-12 flex flex-col w-[323px] px-4 pt-[17px] pb-6 bg-white shadow-[2px_4px_14px_2px_rgba(0,0,0,0.25)] rounded-[10px]">
      <div className="flex justify-between items-center pt-4 w-[289px] max-h-4">
        <h3 className="flex justify-center items-center w-full">Filter</h3>
        <span className="flex items-center cursor-pointer">
          <IoClose onClick={() => setFilterVisible(false)} />
        </span>
      </div>
      <div className="w-full pt-2 mt-2 mb-[14px]">
        <hr />
      </div>
      <div className="">
        {filterData.map((item, index) => (
          <div key={index}>
            {item.name === "Location" && (
              <h3 className="mb-1.5 w-[291px] h-6 font-semibold">{item.name}</h3>
            )}
            {item.name === "Location" && (
              <select
        ...
        {filterData.map((filter, index) => (
          <div className="flex flex-col" key={index}>
            {filter.name !== "Location" && (
              <div
                className={`flex w-[291px] h-6 flex-row items-center justify-between cursor-pointer mt-6`}
                onClick={() => handleFilterClick(index)}
              >
                <label className="cursor-pointer font-semibold">{filter.name}</label>
